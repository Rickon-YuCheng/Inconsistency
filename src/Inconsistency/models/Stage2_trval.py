...
Stage2_trval.py
================
Stage2_official.py 的 train→dev 單次訓練版（拿掉 kfold）。
直接用官方 train（107人）training，官方 dev（35人）當唯一 evaluation，
這樣才能跟其他 DAIC-WOZ paper 的標準 benchmark 數字直接比較。

跟 Stage2_official.py 的差別：
  1. 移除 StratifiedKFold 和 fold 迴圈邏輯
  2. 直接用 get_Split_and_GroundTrue() 拿官方 train(107)/dev(35)
  3. ckpt 命名與 save_dir 改用 stage2_trval 前綴
  4. 保留 oracle threshold 觀察功能（但不作為正式報告指標）
  其餘（model 架構：attention pooling, Stage1_seg_bin_daic, d_model=256）完全不動

uv run src/Inconsistency/models/Stage2_trval.py \
    --stage1_ckpt <你的 Stage1_seg_bin_daic ckpt 路徑> \
    --d_model 256 --enc_layers 1 \
    --lr 1e-4 --weight_decay 1e-4 \
    --dropout 0.3 --atei_dropout 0.3 \
    --lambda_atei 0.7 --alpha_init 0.5 \
...
def parse_args():
    ...
    p.add_argument("--save_dir", type=str, default="weights/stage2_trval")
    ...
    p.add_argument("--wandb_project", type=str, default="Stage2 trval")
...
# ============================================================
# Model (identical to Stage2_official.py)
# ============================================================
class whole_model(nn.Module):
    def __init__(self, embd_size, nheads, atei_ckpt_path=None,
        ...
        if not no_atei:
            ...
            missing, unexpected = self.atei.load_state_dict(sd, strict=False)
            if missing:
                print(f"[ATEI] missing keys (ok if minor): {missing}")
            if unexpected:
                print(f"[ATEI] unexpected keys ignored (e.g. soft-head): {unexpected}")
    ...
    def forward(self, xa, xt, aMask=None, tMask=None, alpha_gate=1.0):
        ...
        if not self.no_atei:
            ...
            if self.training and self.print_norm:
                eT_norm = eT.norm(dim=-1).mean().item() if eT is not None else float('nan')
...
# ============================================================
# Sample Index / Dataset / Collate (identical to Stage2_official.py)
# ============================================================
...
class Stage2SegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if li >= len(xa_list) or li >= len(xt_list):
            raise IndexError(f"DAIC {s['patient_id']} seg {s['seg_id']} "
                             f"list_idx {li} oob")
        return {
...
# ============================================================
# Train (identical to Stage2_official.py)
# ============================================================
def train_one_epoch(model, loader, loss_dep_none, loss_atei, opt, scaler,
                    device, epoch, tot_epochs, cur_lambda,
    ...
    pbar = tqdm(loader, desc=f"Train {epoch}/{tot_epochs}",
...
# ============================================================
# Validate (identical to Stage2_official.py)
# ============================================================
...
def validate(model, loader, loss_dep, device):
    ...
    for batch in tqdm(loader, desc="Dev", unit="batch", leave=False):
        xa    = batch["xa"].to(device, non_blocking=True)
        xt    = batch["xt"].to(device, non_blocking=True)
        aMask = batch["aMask"].to(device, non_blocking=True)
        tMask = batch["tMask"].to(device, non_blocking=True)
...
# ============================================================
# Main — 拿掉 kfold, 固定 train(107) -> dev(35)
# ============================================================
def main():
    ...
    print(f"[Split] official train={len(train_pids)}  official dev={len(dev_pids)}")

    train_idx = Stage2SegIndex(train_pids, daic_depMap,
                               ARGS.daic_pseudo, feat_dir=ARGS.feat_dir)
    dev_idx   = Stage2SegIndex(dev_pids,   daic_depMap,
                               ARGS.daic_pseudo, feat_dir=ARGS.feat_dir)
    print(f"[train] dep dist: {train_idx.get_dep_counts()}")
    print(f"[dev]   dep dist: {dev_idx.get_dep_counts()}")
    ...
    dev_ds   = Stage2SegDataset(dev_idx,   feat_dir=ARGS.feat_dir,
                                cache_size=ARGS.cache_size)

    g = torch.Generator(); g.manual_seed(ARGS.seed)
    sampler = build_dep_sampler(train_idx, seed=ARGS.seed) \
    ...
    run_name = ARGS.wandb_name or (
        f"stage2_trval_seed{ARGS.seed}_lr{ARGS.lr:.0e}_"
        f"la{ARGS.lambda_atei:.2f}_d{ARGS.d_model}_{run_id}")
    if ARGS.use_wandb:
        wandb.init(project=ARGS.wandb_project, name=run_name,
                   config={**vars(ARGS), "primary_metric": "pat_macro_f1"})
    ...
    for epoch in range(1, ARGS.epochs + 1):
        ...
        if v["oracle_macro_f1"] > best_oracle:
            ...
            best_oracle_thr = v["oracle_thr"]

        if v["pat_macro_f1"] > best_pat_macro:
            best_pat_macro = v["pat_macro_f1"]
            no_improve = 0
            if best_pat_macro > ARGS.min_save_f1:
                ckpt = (f"stage2_trval_{run_id}_seed{ARGS.seed}_"
                        f"macroF1_{best_pat_macro:.4f}_binF1_{best_pat_bin:.4f}_"
                        f"ep{epoch:03d}_lr{ARGS.lr:.0e}_d{ARGS.d_model}.pt")
                torch.save({
                ...
                print(f"[Save] MacroF1={best_pat_macro:.4f} BinF1={best_pat_bin:.4f} -> {ckpt}")
        else:
            no_improve += 1
            print(f"[EarlyStop] no macro F1 improvement: {no_improve}/{ARGS.patience}")
        ...
        if ARGS.use_wandb:
            wandb.log({
                ...
                "dev/pat_macro_f1":    v["pat_macro_f1"],
                "dev/pat_bin_f1":      v["pat_bin_f1"],
                "dev/pat_acc":         v["pat_acc"],
                "dev/pat_pre":         v["pat_pre"],
                "dev/pat_rec":         v["pat_rec"],
                "dev/oracle_macro_f1": v["oracle_macro_f1"],
        ...
        if no_improve >= ARGS.patience:
            print(f"[EarlyStop] stop at ep {epoch}. "
                  f"best MacroF1={best_pat_macro:.4f}  BinF1={best_pat_bin:.4f}")
            break
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULT (official train -> official dev, single run)")
    print("=" * 60)
    print(f"Best patMacroF1     : {best_pat_macro:.4f}")
    print(f"Best patBinF1       : {best_pat_bin:.4f}")
    print(f"Best oracleMacroF1 (僅供觀察) : {best_oracle:.4f}")
    print(f"\nTotal time: {timer}")
