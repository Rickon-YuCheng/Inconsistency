...
Reproduce paper settings: d_model=1024, enc_layers=12, 5-fold.
"""
import os
...
def parse_args():
    ...
    p.add_argument("--save_dir", type=str, default="weights/stage1_raw")
    ...
    p.add_argument("--wandb_project", type=str, default="Stage1 raw")
...
class atei(nn.Module):
    def __init__(self, embd_size, nheads, inp_dim=1024, dropout=0.4, enc_layers=12):
        super().__init__()
...
def run_one_fold(fold_id, daic_train_pids, daic_val_pids, run_id, device):
    ...
    run_name = (ARGS.wandb_name + f"_fold{fold_id}") if ARGS.wandb_name else (
        f"stage1_raw_seed{ARGS.seed}_lr{ARGS.lr:.0e}_bs{ARGS.batch_size}_"
    ...
    for epoch in range(1, ARGS.epochs + 1):
        ...
        print(f"[Val]   loss={vr['loss']:.4f} acc={vr['acc']:.4f} "
              f"binF1={vr['bin_f1']:.4f} macroF1={vr['macro_f1']:.4f}")
        ...
        if vr["bin_f1"] > best_bin_f1:
            ...
            ckpt_name = (
                f"stage1_raw_{run_id}_seed{ARGS.seed}_fold{fold_id}_"
...
def main():
    ...
    print("\n" + "="*60)
    print("K-FOLD RESULT (Stage1 raw)")
    print("="*60)
