"""
f10.60
uv run src/Inconsistency/models/Stage2Main_quick.py --epochs 300 --enc_layers 1 --d_model 256 --dropout 0.3 --atei_dropout 0.3 --weight_decay 0 --lr 1e-3 --lambda_atei 0.1 --alpha_init 0.5 --batch_size
 8 --patience 150

f10.66 ep113
uv run src/Inconsistency/models/Stage2Main_quick.py --epochs 300 --enc_layers 1 --d_model 256 --dropout 0.3 --atei_dropout 0.3 --weight_decay 0 --lr 5e-4 --lambda_atei 0.1 --alpha_init 0.5 --batch_size 8 --accum_steps 8 --patience 150
...
from Stage1Tr_quick import atei   # ← 注意:從 _quick 載入
...
# 改成你新訓出來的 Stage1-quick ckpt 路徑
STAGE1_CKPT = "weights/stage1-quick/stage1-quick_20260520_063321_seed42_f10.7424_ep059_lr1e-04_wd1e-04_d256_l1.pt"  # ← 改這行
# STAGE1_CKPT = "weights/stage1-quick/stage1-quick_20260521_083251_seed42_f10.6758_ep023_lr1e-04_wd1e-04_d256_l1.pt" # av
D_MODEL = 256
NHEAD = 8
LR = 5e-4
EPOCHS = 3000
...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="weights/stage2_quick")
    parser.add_argument("--seed", type=int, default=42)

    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="Emotion inconsistency _ Stage2 quick")
...
class whole_model(nn.Module):
    def __init__(self, embd_size=D_MODEL, nheads=NHEAD):
        ...
        # 注意:Stage1Tr_quick 訓的時候是 d_model=256
        self.atei = atei(
            embd_size=256, nheads=nheads,
            dropout=ARGS.atei_dropout,
            enc_layers=1,   # ← Stage1_quick 是 enc_layers 不是 TRANSFORMER_ENC_LAYERS
        )
        ...
        self.aux_a_head = nn.Linear(embd_size, 3)
        self.aux_t_head = nn.Linear(embd_size, 3)
        self.aux_e_head = nn.Linear(embd_size, 3)
...
class stage2_dataset(Dataset):
    def __init__(self, fold: str = "tr", cv_split=None):
        self.ds = []
        a_root = Path("datasets/Feature/HuBERT_quick")       # ← pooled
        t_root = Path("datasets/Feature/RoBerTa_slow")      # ← mental-roberta
        ...
        # PseudoLabel = np.load("PseudoLabel_all_contrastive_q30_70.npz")
...
def stage2_collate_fn(batch):
    """
    每個 sample:
      xa_i: list of [1024]      ← segment-level audio (HuBERT2)
...
def main():
    ...
    for split in splits:
        ...
        run_name = ARGS.wandb_name + f"_fold{fold_id}" if ARGS.wandb_name else (
            f"stage2-quick_seed{ARGS.seed}_lr{LR:.0e}_wd{ARGS.weight_decay:.0e}_"
        ...
        if ARGS.use_wandb:
            wandb.init(
                ...
                config={
                    ...
                    "text_feature": "RoBerTa_slow (token-level)",
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            if val_result["f1"] > best_val_f1:
                ...
                if best_val_f1 > 0.40:
                    ckpt_name = (
                        f"stage2-quick_{run_id}_seed{ARGS.seed}_"
