...
ATEI ckpt
---------
  從 Stage1_seg_bin_daic_eatd 訓出的 ckpt 載入 self.atei。
  ATEI forward(xa, xt, aMask, tMask) - segment-level, frame/token 序列。

Val
---
  純 DAIC kfold val (segment-level → patient majority vote → patient F1)。
  EATD t_* 全部固定加入 train。
"""
...
EATD_PSEUDO = "SegPseudoLabel_eatd_distilbert_pair_bin.npz"
FEAT_DIR = "datasets/Feat_seg_bin_daic_eatd"
EATD_DIR = Path("datasets/EATD")
DAIC_DS_ROOT = "datasets/DAICWOZ"
EATD_SDS_CUTOFF = 53.0


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--stage1_ckpt", type=str, default="weights/stage1_seg_bin_daic_eatd/stage1seg_de_20260603_112601_seed42_fold2_best_macrof1_0.7542_ep009_lr1e-04_d256_l1.pt",
                   help="Stage1_seg_bin_daic_eatd ckpt path")
    ...
    p.add_argument("--eatd_pseudo", type=str, default=EATD_PSEUDO)
    p.add_argument("--feat_dir", type=str, default=FEAT_DIR)
    p.add_argument("--save_dir", type=str, default="weights/stage2_seg_bin_daic_eatd")
    ...
    p.add_argument("--eatd_loss_scale", type=float, default=0.5,
                   help="EATD sample dep+aux loss 權重 (1=同等, 0=完全忽略)")
    p.add_argument("--no_eatd", action="store_true",
                   help="不用 EATD, DAIC only")
    p.add_argument("--use_wandb", action="store_true")
    p.add_argument("--wandb_project", type=str,
                   default="Stage2 seg_bin daic+eatd")
...
def get_eatd_depmap(eatd_dir: Path, cutoff: float = EATD_SDS_CUTOFF):
    ...
    for vol_dir in eatd_dir.iterdir():
        if not vol_dir.is_dir():
            continue
        if not (vol_dir.name.startswith("t_") or vol_dir.name.startswith("v_")):
            continue
        lbl = vol_dir / "new_label.txt"
        if not lbl.exists():
            continue
        try:
            val = float(lbl.read_text().strip())
            depmap[vol_dir.name] = 1 if val >= cutoff else 0
        except ValueError:
            continue
    return depmap
            ...
            for vol in eatd_vols:
                ...
                for seg_id in [0, 1, 2]:
                    ...
                    self.samples.append({
                        ...
                        "seg_id": seg_id,
                        "list_idx": seg_id,
                        "dep_label": eatd_depMap[vol],
                        "atei_label": atei_map[key],
                    })
...
class Stage2SegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if li >= len(xa_list) or li >= len(xt_list):
            raise IndexError(f"{s['corpus']} {pid} seg {s['seg_id']} "
                             f"list_idx {li} oob")
        return {
...
def train_one_epoch(model, loader, loss_dep_none, loss_atei, opt, scaler,
    ...
    for step, batch in enumerate(pbar):
        ...
        with torch.autocast(device_type="cuda", enabled=(device == "cuda"),
            ...
            L_dep = (loss_dep_none(dep_logits, dep) * sample_scale).mean()
            if (atei_lab != -1).any():
                L_atei = loss_atei(atei_logits, atei_lab)
            else:
                L_atei = torch.tensor(0.0, device=device)
            L_aux = ((loss_dep_none(aux_a, dep) * sample_scale).mean()
                     + (loss_dep_none(aux_t, dep) * sample_scale).mean()
                     + (loss_dep_none(aux_e, dep) * sample_scale).mean()) / 3.0
...
def validate(model, loader, loss_dep, device, fold_id):
    ...
    pat_true, pat_pred = [], []
    for pid, scs in per_patient_scores.items():
        pat_pred.append(int(np.mean(scs) >= 0.0))
...
def run_one_fold(fold_id, daic_train_pids, daic_val_pids, eatd_train_vols,
    ...
    print(f"\n{'='*60}\nFOLD {fold_id}\n{'='*60}")
    eatd_npz = None if ARGS.no_eatd else ARGS.eatd_pseudo
    train_idx = Stage2SegIndex(daic_train_pids, eatd_train_vols,
                                daic_depMap, eatd_depMap,
                                ARGS.daic_pseudo, eatd_npz,
                                feat_dir=ARGS.feat_dir)
    val_idx = Stage2SegIndex(daic_val_pids, [], daic_depMap, eatd_depMap,
                              ARGS.daic_pseudo, None, feat_dir=ARGS.feat_dir)
