"""
uv run src/Inconsistency/models/Stage2_2022.py --d_model 256 --enc_layers 1 --lr 1e-4 --weight_decay 0 --dropout 0.5 --batch_size 64 --epochs 30 --kfold 3 --use_class_weight

Stage2_2022.py
==============
仿照 ICASSP2022 (Shen et al.) 的 EATD evaluation protocol，套到 DAIC-WOZ：

[2022 做法（從 audio_gru_whole.py 觀察到的）]
  - 官方 train set 做 StratifiedKFold 3 折，切出 tr / val
  - val 同時扮演 test（val=test，每 epoch 都看，用來決定存 checkpoint）
  - max_f1 是全域變數，三折共用，只存三折裡全局最好的 checkpoint
  - 最終報 global best f1（不是 mean f1）

[與 Stage2_official.py 的差異]
  [CHANGED] val=test，每 epoch evaluate val 並用 val f1 決定存 checkpoint
  [CHANGED] 每折各自有 fold_best，不跨折共用（仿照 2022 max_f1 每折 reset）
  [CHANGED] depressed permutation augmentation on both train AND test（仿照 2022）
  [CHANGED] 最終報每折各自最好的 f1，不是全局最好也不是 mean
  [CHANGED] 使用官方 train set 做 kfold（官方 dev 完全不碰）
  [KEPT]    majority vote (patient-level)，比 2022 更合理
  [KEPT]    segment-level HuBERT feat + Transformer，非 GRU
  [REMOVED] patience / early stopping（2022 跑滿 epochs）
  [ADDED]   --encoder_type {attn, hope_attention}，移植自 Stage2_official_search.py
            --cms_periods P1 P2
  [NOTE]    2022 test set 也做 depressed augmentation（4種排列），這裡也照做
"""

import os
os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
# [ADDED] sm7.5 (T4) 不支援 flash/memory-efficient attention
# 強制使用 math kernel，避免 hope_attention 的 sdpa 找不到 kernel
...
from Inconsistency.models.hope_adapter import HopeEncoderBlock  # [ADDED]
...
N_CLASSES    = 2
MIN_SAVE_F1  = 0.40
ENCODER_TYPE = "attn"           # [ADDED] "attn" | "hope_attention"
CMS_PERIODS  = [1, 4]           # [ADDED] hope_attention 用
...
def parse_args():
    ...
    p.add_argument("--save_dir",         type=str,   default="weights/stage2_2022")
    ...
    p.add_argument("--wandb_project",    type=str,   default="Stage2 2022")
    ...
    p.add_argument("--phase1_dir",       type=str,   default=None,
                   help="Phase1 存的 train_pids_*.json 資料夾，不指定則直接 KFold")
    # [ADDED] encoder type
    p.add_argument("--encoder_type",     type=str,   default=ENCODER_TYPE,
                   choices=["attn", "hope_attention"])
    p.add_argument("--cms_periods",      type=int,   nargs=2,
                   default=CMS_PERIODS)
    return p.parse_args()


# ============================================================
# Model (same as Stage2_official.py)
# ============================================================
class whole_model(nn.Module):
    def __init__(self, embd_size, nheads, atei_ckpt_path=None,
                 ...
                 encoder_type="attn", cms_periods=(1, 4)):  # [ADDED]
        super().__init__()
        self.print_norm   = print_norm
        self.no_atei      = no_atei
        self.no_text      = no_text
        self.encoder_type = encoder_type  # [ADDED]
        ...
        # [CHANGED] encoder 根據 encoder_type 決定用 Transformer 或 HopeEncoderBlock
    ...
    def forward(self, xa, xt, aMask=None, tMask=None, alpha_gate=1.0):
        ...
        HA = self._encode(XA, self.a_encoder, aMask)  # [CHANGED] 用 _encode
        ...
        if not self.no_text:
            ...
            HT = self._encode(XT, self.t_encoder, tMask)  # [CHANGED] 用 _encode
...
class Stage2SegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if li >= len(xa_list) or li >= len(xt_list):
            raise IndexError(
                f"DAIC {s['patient_id']} seg {s['seg_id']} list_idx {li} oob")
        return {
...
# ============================================================
# Depressed permutation augmentation
# [ADDED] 仿照 2022：depressed segments 做 permutation augmentation
#         train 用 6 種排列，test 用 4 種排列
# ============================================================
def augment_depressed_segments(sample_list, dep_label=1,
                               n_perms=6, seed=0):
    """
    對 dep_label==1 的 patient 的 segments 做 permutation augmentation。
    每個 depressed patient 的所有 segments 順序做 n_perms 種排列，
    產生新的 samples 加入 sample_list。
    回傳擴增後的新 list（不修改原本的）。

    仿照 2022：train 用 n_perms=6，test 用 n_perms=4。
    """
    import itertools
    rng = np.random.RandomState(seed)
    new_samples = list(sample_list)  # copy

    # 按 patient 分組
    from collections import defaultdict
    pat_segs = defaultdict(list)
    for s in sample_list:
        if s["dep_label"] == dep_label:
            pat_segs[s["patient_id"]].append(s)

    for pid, segs in pat_segs.items():
        n = len(segs)
        if n <= 1:
            continue
        # 對 segment 的 list_idx 順序做排列
        perm_count = 0
        for perm in itertools.permutations(range(n)):
            if perm_count >= n_perms:
                break
            if perm == tuple(range(n)):  # skip identity
                continue
            for new_pos, orig_pos in enumerate(perm):
                orig_seg = segs[orig_pos]
                new_seg = dict(orig_seg)
                # 保留原本的 dep/atei label，只換 list_idx 對應位置
                new_seg = {
                    "patient_id": orig_seg["patient_id"],
                    "seg_id":     orig_seg["seg_id"],
                    "list_idx":   segs[new_pos]["list_idx"],  # 換位置
                    "dep_label":  orig_seg["dep_label"],
                    "atei_label": orig_seg["atei_label"],
                }
                new_samples.append(new_seg)
            perm_count += 1

    print(f"[augment] {len(sample_list)} -> {len(new_samples)} "
          f"(+{len(new_samples)-len(sample_list)}, n_perms={n_perms})")
    return new_samples
...
def train_one_epoch(model, loader, loss_dep_none, loss_atei, opt, scaler,
    ...
    pbar = tqdm(loader,
                desc=f"Fold{fold_id} Train {epoch}/{tot_epochs}",
    ...
    for step, batch in enumerate(pbar):
        ...
        if atei_logits is not None:
            correct_atei += (
                (atei_logits.argmax(-1) == atei_lab) & valid_atei_mask
            ).sum().item()
        ...
        n        += dep.size(0)
        pbar.set_postfix({"dep": tot_dep/max(n,1), "acc": correct_dep/max(n,1)})
    ...
    return {"dep_loss":  tot_dep  / max(n, 1),
...
# ============================================================
# Evaluate
# [CHANGED] val=test，每 epoch 都跑
#           fold_best 每折 reset，仿照 2022 max_f1
# ============================================================
@torch.inference_mode()
def evaluate_and_maybe_save(model, loader, loss_dep, device, fold_id,
                             epoch, run_id, save_dir, fold_best):
    # [CHANGED] fold_best 每折獨立，不跨折共用（仿照 2022 max_f1 每折 reset）
    model.eval()
    ...
    for batch in tqdm(loader,
                      desc=f"Fold{fold_id} Val/Test ep{epoch}",
                      unit="batch", leave=False):
        xa    = batch["xa"].to(device,    non_blocking=True)
        xt    = batch["xt"].to(device,    non_blocking=True)
        aMask = batch["aMask"].to(device,  non_blocking=True)
    ...
    pat_macro = f1_score(pat_true, pat_pred, average="macro",
                         labels=[0, 1], zero_division=0)
    pat_bin   = f1_score(pat_true, pat_pred, average="binary",
                         pos_label=1, zero_division=0)
    pat_acc   = float((pat_true == pat_pred).mean())
    pat_rec   = recall_score(pat_true, pat_pred, average="binary",
                             pos_label=1, zero_division=0)
    pat_pre   = precision_score(pat_true, pat_pred, average="binary",
                                pos_label=1, zero_division=0)

    # [CHANGED] 仿照 2022：每折獨立的 fold_best，max_f1 每折 reset
    if pat_macro > fold_best["f1"] and pat_macro > ARGS.min_save_f1:
        fold_best["f1"]   = pat_macro
        fold_best["acc"]  = pat_acc
        fold_best["rec"]  = pat_rec
        fold_best["prec"] = pat_pre
        ckpt_name = (
            f"stage2_2022_{run_id}_seed{ARGS.seed}_fold{fold_id}_"
            f"foldBest_macroF1_{pat_macro:.4f}_ep{epoch:03d}_"
            f"lr{ARGS.lr:.0e}_d{ARGS.d_model}.pt")
        torch.save({
            "model_state_dict": model.state_dict(),
            "epoch":          epoch,
            "fold":           fold_id,
            "pat_macro_f1":   pat_macro,
            "pat_bin_f1":     pat_bin,
            "pat_acc":        pat_acc,
            "args":           vars(ARGS),
            "selected_by":    "fold_pat_macro_f1",
        }, save_dir / ckpt_name)
        fold_best["ckpt"] = ckpt_name
        print(f"[Save fold-best] fold{fold_id} macroF1={pat_macro:.4f} -> {ckpt_name}")
...
# ============================================================
# Per-fold runner
# [CHANGED] fold_best 每折獨立 reset，不跨折共用
# [REMOVED] patience / early stopping（仿照 2022，跑滿 epochs）
# ============================================================
def run_one_fold(fold_id, train_pids, val_pids,
                 daic_depMap, run_id, device):
    # [CHANGED] 移除 global_best，改用 fold_best（每折 reset，仿照 2022）
    ...
    # [CHANGED] val=test，同一批資料既當 val 又當 test（仿照 2022）
    val_idx   = Stage2SegIndex(val_pids,   daic_depMap,
                               ARGS.daic_pseudo, feat_dir=ARGS.feat_dir)
    print(f"[train]    dep dist: {train_idx.get_dep_counts()}")
    print(f"[val/test] dep dist: {val_idx.get_dep_counts()}")

    # [ADDED] 仿照 2022：depressed permutation augmentation
    # train 用 6 種排列，test 用 4 種排列
    train_samples_aug = augment_depressed_segments(
        train_idx.samples, dep_label=1, n_perms=6, seed=ARGS.seed + fold_id)
    val_samples_aug   = augment_depressed_segments(
        val_idx.samples,   dep_label=1, n_perms=4, seed=ARGS.seed + fold_id + 100)

    # 用 augmented samples 建 dataset（直接替換 .samples）
    train_idx_aug = type("AugIndex", (), {"samples": train_samples_aug})()
    val_idx_aug   = type("AugIndex", (), {"samples": val_samples_aug})()

    train_ds = Stage2SegDataset(train_idx_aug, feat_dir=ARGS.feat_dir,
                                cache_size=ARGS.cache_size)
    val_ds   = Stage2SegDataset(val_idx_aug,   feat_dir=ARGS.feat_dir,
                                cache_size=ARGS.cache_size)

    g       = torch.Generator(); g.manual_seed(ARGS.seed + fold_id)
    sampler = build_dep_sampler(train_idx_aug, seed=ARGS.seed + fold_id) \
    ...
    model = whole_model(
        ...
        cms_periods=tuple(ARGS.cms_periods)).to(device)  # [ADDED]
    ...
    run_name = (
        (ARGS.wandb_name + f"_fold{fold_id}") if ARGS.wandb_name
        else (f"stage2_2022_seed{ARGS.seed}_lr{ARGS.lr:.0e}_"
              f"la{ARGS.lambda_atei:.2f}_d{ARGS.d_model}"
              f"_fold{fold_id}_{run_id}")
    )
    if ARGS.use_wandb:
        wandb.init(project=ARGS.wandb_project, name=run_name, reinit=True,
                   config={**vars(ARGS), "fold": fold_id})

    # [CHANGED] fold_best 每折 reset（仿照 2022 max_f1 每折 reset）
    fold_best = {"f1": -1.0, "acc": -1.0, "rec": -1.0, "prec": -1.0, "ckpt": None}

    for epoch in range(1, ARGS.epochs + 1):
        alpha_gate = (min(1.0, epoch / ARGS.alpha_warmup)
                      if ARGS.alpha_warmup > 0 else 1.0)
        cur_lambda = (0.0 if ARGS.no_atei_loss else
                      (ARGS.lambda_atei *
                       min(1.0, epoch / ARGS.lambda_warmup)
                       if ARGS.lambda_warmup > 0 else ARGS.lambda_atei))

        print("=" * 80)
        alpha_str = (f"alpha={float(model.alpha.detach()):.4f} "
                     if not ARGS.no_atei else "")
        print(f"[Fold {fold_id}] Epoch [{epoch}/{ARGS.epochs}]  "
              f"{alpha_str}lambda={cur_lambda:.4f}  "
              f"fold_best={fold_best['f1']:.4f}")
        ...
        # [CHANGED] val 同時當 test，每 epoch 都跑，更新 fold_best
        v = evaluate_and_maybe_save(
            model, val_loader, loss_dep, device, fold_id,
            epoch, run_id, save_dir, fold_best)

        print(f"[Train] dep_loss={tr['dep_loss']:.4f} "
              f"dep_acc={tr['dep_acc']:.4f}")
        print(f"[Val/Test] n={v['n_patients']} "
              f"acc={v['pat_acc']:.4f} "
              f"binF1={v['pat_bin_f1']:.4f} "
        ...
        print(classification_report(
            v["pat_true"], v["pat_pred"], labels=[0, 1],
            target_names=["healthy(0)", "depressed(1)"],
            digits=4, zero_division=0))
        print(f"[Fold {fold_id} best so far] macroF1={fold_best['f1']:.4f}")
...
# ============================================================
# Main
# [CHANGED] fold_best 每折獨立，main 收集每折結果後報 best fold
# ============================================================
def main():
    ...
    # 仿照 2022 第二階段：載入 phase1 存好的 train_pids json
    # 若沒有指定 phase1_dir，則直接用 KFold（等價但少了兩階段的形式）
    daic_depMap, train_pids, dev_pids = get_Split_and_GroundTrue()

    if ARGS.phase1_dir:
        import glob
        json_files = sorted(glob.glob(
            str(Path(ARGS.phase1_dir) / "train_pids_*.json")))
        if not json_files:
            raise FileNotFoundError(
                f"No train_pids_*.json found in {ARGS.phase1_dir}")
        print(f"[Phase2] loading {len(json_files)} train_pids from {ARGS.phase1_dir}")
        folds = []
        for i, jf in enumerate(json_files):
            ...
            print(f"  {Path(jf).name}  f1={d.get('f1',-1):.4f}  "
                  f"train={len(d['train_pids'])}p  val={len(d['val_pids'])}p")
    else:
        # fallback: 直接 KFold（同 phase1 的行為，只是不存 json）
        all_pids = np.array(train_pids + dev_pids)
        labels   = np.array([daic_depMap[p] for p in all_pids])
        skf   = StratifiedKFold(n_splits=ARGS.kfold, shuffle=True,
                                 random_state=ARGS.seed)
        folds = []
        for fold_id, (tr_idx, val_idx) in enumerate(skf.split(all_pids, labels)):
            folds.append({
                "fold":  fold_id,
                "train": all_pids[tr_idx].tolist(),
                "val":   all_pids[val_idx].tolist(),
            })

    # [CHANGED] 每折各自有 fold_best，仿照 2022 每折 reset
    fold_results = []
    for f in folds:
        fb = run_one_fold(f["fold"], f["train"], f["val"],
                          daic_depMap, run_id, device)
        fold_results.append(fb)
        print(f"\n>>> Fold {f['fold']} best macroF1={fb['f1']:.4f}  "
              f"acc={fb['acc']:.4f}  rec={fb['rec']:.4f}  prec={fb['prec']:.4f}")

    print("\n" + "=" * 60)
    print("RESULT (Stage2 2022 protocol, per-fold best)")
    print("=" * 60)
    for i, fb in enumerate(fold_results):
        print(f"Fold {i}: macroF1={fb['f1']:.4f}  acc={fb['acc']:.4f}  "
              f"rec={fb['rec']:.4f}  prec={fb['prec']:.4f}")
        if fb["ckpt"]:
            print(f"         ckpt: {fb['ckpt']}")

    # 2022 報 best fold（不是 mean）
    best_fold = max(fold_results, key=lambda x: x["f1"])
    print(f"\n[Best Fold macroF1] = {best_fold['f1']:.4f}")
    print(f"  acc={best_fold['acc']:.4f}  "
          f"rec={best_fold['rec']:.4f}  "
          f"prec={best_fold['prec']:.4f}")
    print(f"\nTotal time: {timer}")
