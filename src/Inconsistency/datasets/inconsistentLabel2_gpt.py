...
def make_zdist_pseudo_label(
    ...
    if text_source == "hownet":
        T_counts, TpatientIdx = load_emotion_counts(f"HowNet_{split}.npz")
    elif text_source == "distilbert":
        T_counts, TpatientIdx = load_emotion_counts(f"DistilBert_{split}.npz")
    else:
        raise ValueError(f"unknown text_source: {text_source}")

    A_counts, ApatientIdx = load_emotion_counts(f"Wav2Vec2_{split}.npz")
    ...
    out_path = f"PseudoLabel_{split}_{text_source}_zdist_q{low_q}_{high_q}.npz"
