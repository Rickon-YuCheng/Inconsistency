...
def parse_args():
    ...
    p.add_argument("--save_dir", type=str, default="weights/stage2_raw")
    ...
    p.add_argument("--wandb_project", type=str, default="Stage2 raw")
...
def validate(model, loader, loss_dep, device, fold_id):
    ...
    # majority vote: argmax of mean logit score
...
def run_one_fold(fold_id, daic_train_pids, daic_val_pids,
    ...
    run_name = (ARGS.wandb_name + f"_fold{fold_id}") if ARGS.wandb_name else (
        f"stage2_raw_seed{ARGS.seed}_lr{ARGS.lr:.0e}_"
    ...
    for epoch in range(1, ARGS.epochs + 1):
        ...
        if v["pat_bin_f1"] > best_pat_bin:
            ...
            if best_pat_bin > ARGS.min_save_f1:
                ckpt = (f"stage2_raw_{run_id}_seed{ARGS.seed}_fold{fold_id}_"
...
def main():
    ...
    print("\n" + "="*60)
    print("K-FOLD RESULT (Stage2 raw)")
    print("="*60)
