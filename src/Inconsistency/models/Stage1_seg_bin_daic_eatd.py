"""
Stage1_seg_bin_daic_eatd.py
============================
Stage1 ATEI segment-level training, joint DAIC + EATD.

Mix
---
  Stage1Tr_seg_bin.py    -> segment-level dataloader / training loop
  Stage1_daic_eatd.py    -> DAIC+EATD joint split, 雙獨立 encoder ATEI

Pseudo label
------------
  SegPseudoLabel_daic_distilbert_pair_bin.npz   (DAIC, patientIdx: int)
  SegPseudoLabel_eatd_distilbert_pair_bin.npz   (EATD, patientIdx: str)

Feature
-------
  datasets/Feat_seg_bin_daic_eatd/  (DAIC + EATD full-frame audio + token text)

Val
---
  純 DAIC kfold val。EATD t_* 全部固定加入 train pool。
"""
import os
...
EATD_PSEUDO_NPZ = "SegPseudoLabel_eatd_distilbert_pair_bin.npz"
FEAT_DIR = "datasets/Feat_seg_bin_daic_eatd"
...
def parse_args():
    ...
    p.add_argument("--eatd_pseudo_npz", type=str, default=EATD_PSEUDO_NPZ)
    p.add_argument("--feat_dir", type=str, default=FEAT_DIR)
    p.add_argument("--save_dir", type=str, default="weights/stage1_seg_bin_daic_eatd")
    ...
    p.add_argument("--wandb_project", type=str, default="Stage1 seg_bin daic+eatd")
    p.add_argument("--wandb_name", type=str, default=None)
    p.add_argument("--kfold", type=int, default=3)
    p.add_argument("--no_eatd", action="store_true",
                   help="disable EATD, train DAIC only")
    return p.parse_args()


# ============================================================
# ATEI model (segment-level, 雙獨立 encoder, 跟 Stage1_daic_eatd 對齊)
# ============================================================
class atei(nn.Module):
    def __init__(self, embd_size, nheads, inp_dim=1024, dropout=0.4, enc_layers=1):
        ...
        self.patient_oup = nn.Linear(embd_size, 2)   # 保留, Stage2 不會用但 state_dict 一致
...
# ============================================================
# Sample Index (DAIC + EATD 統一管理)
# ============================================================
class SegSampleIndex:
    def __init__(self, daic_pids, eatd_vols, daic_npz, eatd_npz=None,
        ...
        if daic_pids and daic_npz:
            ...
            for pid in daic_pids:
                ...
                if not (a_path.exists() and t_path.exists()):
                    ...
                    continue
                # 用 mmap 載入只是為了拿長度,不會真的把 tensor 全讀進記憶體
                ...
                for list_idx, row in enumerate(df_p.itertuples()):
                    ...
                    self.samples.append({
                        ...
                        "seg_id": seg_id,
                        "list_idx": list_idx,
                        "atei_label": d_map[key],
                    })

        # ---------- EATD (不變) ----------
        if eatd_vols and eatd_npz:
            epl = np.load(eatd_npz, allow_pickle=True)
            e_pid = epl["patientIdx"]
            e_sid = epl["segIdx"].astype(np.int64)
            e_lab = epl["label"].astype(np.int64)
            vol_set = set(eatd_vols)
            for p, s, l in zip(e_pid, e_sid, e_lab):
                vol = str(p)
                if vol not in vol_set:
                    continue
                self.samples.append({
                    "corpus": "eatd",
                    "patient_id": vol,
        ...
        n_daic = sum(1 for s in self.samples if s["corpus"] == "daic")
        n_eatd = sum(1 for s in self.samples if s["corpus"] == "eatd")
        print(f"[SegSampleIndex] total={len(self.samples)} (DAIC={n_daic}, EATD={n_eatd})")
    def __len__(self):
...
class SegDataset(Dataset):
    ...
    def _load_patient(self, pid):
        # pid: int (DAIC) or str (EATD), 統一當 key
        if pid in self._cache:
            self._cache_order.remove(pid)
            self._cache_order.append(pid)
            return self._cache[pid]

        a_path = self.feat_dir / f"{pid}_acoustic.pt"
        t_path = self.feat_dir / f"{pid}_text.pt"
        xa = torch.load(str(a_path), map_location="cpu", mmap=True)
        xt = torch.load(str(t_path), map_location="cpu", mmap=True)
        xa_list = [x.squeeze(0) for x in xa]   # each [T, 1024]
        xt_list = [x.squeeze(0) for x in xt]   # each [L, 1024]
    ...
    def __getitem__(self, idx):
        ...
        if li >= len(xa_list) or li >= len(xt_list):
            raise IndexError(
                f"{s['corpus']} {s['patient_id']} seg {s['seg_id']} "
...
def collate_fn(batch):
    xa_list = [b["xa"] for b in batch]
    xt_list = [b["xt"] for b in batch]
    labels = torch.tensor([b["atei_label"] for b in batch], dtype=torch.long)
...
def train_one_epoch(model, loader, criterion, opt, scaler, device, epoch, tot, fold_id):
    ...
    for batch in pbar:
        ...
        n += labels.size(0)
        pbar.set_postfix({"loss": total_loss/max(n,1), "acc": correct/max(n,1)})
...
# Per-fold runner
# ============================================================
def run_one_fold(fold_id, daic_train_pids, daic_val_pids, eatd_train_vols,
                 run_id, device):
    ...
    eatd_npz = None if ARGS.no_eatd else ARGS.eatd_pseudo_npz
    train_idx = SegSampleIndex(daic_train_pids, eatd_train_vols,
                               ARGS.daic_pseudo_npz, eatd_npz, feat_dir=ARGS.feat_dir)
    val_idx   = SegSampleIndex(daic_val_pids, [], ARGS.daic_pseudo_npz, None, feat_dir=ARGS.feat_dir)
    print(f"[train] {len(train_idx)} samples, dist: {train_idx.get_label_counts()}")
    print(f"[val]   {len(val_idx)} samples (DAIC only), dist: {val_idx.get_label_counts()}")
    ...
    run_name = (ARGS.wandb_name + f"_fold{fold_id}") if ARGS.wandb_name else (
        f"stage1seg_de_seed{ARGS.seed}_lr{ARGS.lr:.0e}_bs{ARGS.batch_size}_"
    ...
    for epoch in range(1, ARGS.epochs + 1):
        ...
        if vr["bin_f1"] > best_bin_f1:
            ...
            ckpt_name = (
                f"stage1seg_de_{run_id}_seed{ARGS.seed}_fold{fold_id}_"
        ...
        if ARGS.use_wandb:
            wandb.log({"epoch": epoch,
                       ...
                       "val/loss": vr["loss"], "val/acc": vr["acc"],
                       "val/bin_f1": vr["bin_f1"], "val/macro_f1": vr["macro_f1"],
        ...
        if no_improve >= ARGS.patience:
            print(f"[EarlyStop] Fold {fold_id} stop ep {epoch}, "
                  f"best binF1={best_bin_f1:.4f}, macroF1={best_macro_f1:.4f}")
            break
...
# Helpers
# ============================================================
def get_eatd_train_vols(eatd_npz: str):
    """從 EATD pseudo label npz 拿所有 t_* volunteer ids。"""
    epl = np.load(eatd_npz, allow_pickle=True)
    pids = epl["patientIdx"]
    return sorted({str(p) for p in pids if str(p).startswith("t_")},
...
def main():
    ...
    eatd_train_vols = []
    if not ARGS.no_eatd:
        eatd_train_vols = get_eatd_train_vols(ARGS.eatd_pseudo_npz)
        print(f"[EATD] {len(eatd_train_vols)} train volunteers (t_*)")

    fold_results = []
    for f in folds:
        r = run_one_fold(f["fold"], f["train"], f["val"], eatd_train_vols,
                          run_id, device)
    ...
    print("\n" + "="*60)
    print("K-FOLD RESULT (Stage1 seg_bin daic+eatd)")
    print("="*60)
