'''
f1 0.74 classic
uv run src/Inconsistency/models/Stage1Tr_quick.py --d_model 256 --enc_layers 1 --lr 1e-4 --weight_decay 1e-4 --dropout 0.3 --epochs 100
f1 0.67 Valence / Arousal
uv run src/Inconsistency/models/Stage1Tr_quick.py --d_model 256 --enc_layers 1 --lr 1e-4 --weight_decay 1e-4 --dropout 0.3 --epochs 100
'''
"""
Stage1Tr-quick.py — segment-level ATEI training (use HuBERT2 pooled feature)

跟原 Stage1Tr_v1.py 差別:
- audio path 從 HuBERT 改成 HuBERT2 (pooled)
...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="weights/stage1-quick")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--label_smoothing", type=float, default=0.05)

    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="Emotion inconsistency - Stage1 quick")
...
class daicwoz_dataset(Dataset):
    """
    每個 patient:
      xa_list: list of [1024]  (HuBERT2, pool 過, 每句一個 vector)
    ...
    def __init__(self, fold: str = "tr"):
        ...
        # PseudoLabel = np.load("PseudoLabel_all_va_eci_q30_70.npz")
...
def main():
    ...
    run_name = ARGS.wandb_name or (
        f"stage1_quick_seed{ARGS.seed}_lr{LR:.0e}_wd{ARGS.weight_decay:.0e}"
        f"_do{ARGS.dropout:.2f}_d{D_MODEL}_l{TRANSFORMER_ENC_LAYERS}_{run_id}"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        pbar = tqdm(tr_loader, f"Train Epoch {epoch+1}/{EPOCHS}",
        ...
        if val_result["macro_f1"] > best_val_f1:
            ...
            no_improve = 0
            ckpt_name = (
                f"stage1-quick_{run_id}_seed{ARGS.seed}_"
            ...
            torch.save({
                ...
                "audio_feature": "HuBERT_quick",
                "text_feature": "RoBerTa_slow",
                "atei_level": "segment",
            }, ckpt_path)
...
def imsave(history):
    ...
    plt.title('Training Loss (Stage1-quick)')
    plt.savefig("stage1-quick_tr_loss.jpg")
